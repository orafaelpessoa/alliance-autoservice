...
export default function EditBudgetClient() {
  ...
  useEffect(() => {
  if (!budgetId || parts.length === 0) return;
  ...
  const loadBudget = async () => {
    ...
    setVehicleId(budget.vehicle_id);

    const loadedItems = budget.budget_items.map((item: any) => ({
      type: item.type,
...
}, [budgetId, parts, router, budgetLoaded]);
  ...
  return (
    <main className="min-h-screen bg-gray-100">
      ...
      <section className="max-w-5xl mx-auto px-4 mt-10 space-y-8">
        <header>
          <h1 className="text-2xl font-semibold text-black">Editar orçamento</h1>
